 ...
 * smpte_ltc.h
 *
 * SMPTE/EBU Linear Timecode (LTC) decoder for TI Tiva TM4C + TI-RTOS (SYS/BIOS).
 *
 * Decodes biphase-mark-encoded LTC audio (already squared to a digital
 * 0/3.3V signal by an external comparator ahead of the MCU). The edge
 * timestamp source is hardware-agnostic from this file's point of view --
 * it can be a software read of a free-running timer from a GPIO ISR, or
 * (lower jitter, recommended) a GPTM edge-time hardware capture register.
 * See smpte_hw.c for the CCP capture-based implementation.
 *
 * Supports both playback directions (forward and reverse/rewind) and
 * flags two classes of decode error: mid-stream framing anomalies
 * (badSyncCount) and total signal loss / stalled input (timedOut,
 * driven by a periodic watchdog call -- see SMPTE_LTC_onTimeout()).
 *
 * Reference: SMPTE 12M-1999 / EBU R-alpha timecode format.
 *
 * IMPORTANT: the bit offsets below follow the commonly published SMPTE 12M
 * LTC frame layout, and the reverse-direction sync word was derived
 * analytically (see smpte_ltc.c) rather than lifted from a verified
 * reference implementation. Confirm both against a logic analyzer capture
 * of your own source -- in both directions -- before relying on this in
 * production. This has not been validated against real hardware.
 */
